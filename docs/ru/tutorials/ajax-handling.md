...
evalJS       | true      | автоматически исполнять тексты с типом контента `javascript`
...
jsonp        | false     | `true` или имя перменной для JSONP запросов
...
## Поддержка JSONP, :jsonp

Вы так же можете легко создавать `JSONP` запросы с помощью класса {Xhr}.
В данном случае все выглядит в точности точно так же, просто вы добавляете
опцию `jsonp` указывая нужный тип запроса
...
Значение `true` указывает на использование имени переменной для функции
обратного вызова по умолчанию (`'callback'`), но вы можете указать любое
другое если вам это необходимо
