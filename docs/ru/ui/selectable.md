...
* Все включено в один небольшой (3.4k gzipped) файл, включая базовые стили
...
А так же вы можете назначить на ваш элемент списка css-класс `rui-selectable` и
...
атрибуты опций в стиле HTML5 `data-selectable`, как и с любым другим виджетом
RightJS

И еще вы можете автоматически конвертировать обычные селект-боксы, в поля выбора,
просто назначив им css-класс `rui-selectable`. Скрипт автоматически найдет их,
...
можете создавать поля с одним выбором, указав дополнительный css-класс `rui-selectable-single`
...
`Selectable`, в виде JSON данных в атрибуте `data-selectable` или глобально
...
hCont      | '&amp;bull;' | контент элемента управления списков с одним выбором
...
`rui-selectable-selected` и `rui-selectable-disabled`, соответственно для выделенных
...
Для полей с единичным выбором, скрипт добавит css-класс `rui-selectable-single` и вставит
